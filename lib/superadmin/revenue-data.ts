...
export interface RevenueData {
  ...
  cohort_table: Array<{
    ...
    mrr_now_cents: number;           // current $ generated by surviving tenants
    arpu_cents: number;              // mrr_now / still_active
  }>;
  churn_waterfall: {
    start_count: number;
    new: number;
...
export async function fetchRevenueData(): Promise<RevenueData> {
  ...
  const cohort_table = Array.from(cohorts.entries())
    ...
    .map(([month, info]) => {
      const surviving = info.signed.filter(
        (t) => t.subscription_status === "active" && !t.suspended_at && !t.cancel_at_period_end,
      );
      const stillActive = surviving.length;
      const mrr_now_cents = surviving.filter((t) => t.plan !== "founder").length * 9900;
      return {
        cohort_month: month,
        ...
        retention_pct: info.signed.length > 0
          ...
          : 0,
        mrr_now_cents,
        arpu_cents: stillActive > 0 ? Math.round(mrr_now_cents / stillActive) : 0,
      };
    });
