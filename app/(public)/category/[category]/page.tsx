...
export default async function CategoryPage({
  ...
  // Look up category by slug (driven by DB, not hardcoded)
  ...
  const showComingSoon = !category.is_active || list.length === 0;

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold text-brand-navy mb-2">{category.name}</h1>
        <p className="text-slate-600">
          {showComingSoon
            ? category.description || "Coming soon — new inventory landing soon."
      ...
      {showComingSoon ? (
        <div className="card text-center py-16 max-w-xl mx-auto">
          ...
          <p className="text-slate-600 mb-6">
            New {category.name.toLowerCase()} are on their way. In the meantime,
