...
import { createTenantGoal } from "./actions";
...
export function GoalForm() {
  ...
  function submit(formData: FormData) {
    startTransition(async () => {
      const r = await createTenantGoal(formData);
      if ("error" in r) toast.error(r.error);
      else { toast.success("Goal created"); setOpen(false); }
    });
  }
  ...
  return (
    <form action={submit} className="card border-2 border-amber-300 p-5 space-y-4">
      ...
      <div>
        ...
        <select name="metric" className="input" defaultValue="bookings_30d">
          <option value="bookings_30d">Bookings in last 30 days</option>
          <option value="revenue_30d">Revenue in last 30 days ($)</option>
          <option value="customers_30d">New customers in last 30 days</option>
          <option value="repeat_rate">Repeat customer rate (%)</option>
        </select>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs font-bold uppercase text-slate-500 mb-1">Target value</label>
          <input name="target" type="number" min="1" step="any" required placeholder="30" className="input" />
        </div>
        <div>
      ...
      <div>
        ...
        <input name="label" placeholder="e.g. Summer push" className="input" />
      </div>
      <div className="flex gap-2">
        <button type="submit" disabled={pending} className="bg-amber-500 hover:bg-amber-600 disabled:bg-slate-300 text-white font-semibold rounded-lg px-4 py-2 text-sm">
          {pending ? "Saving…" : "Create goal"}
        </button>
        <button type="button" onClick={() => setOpen(false)} className="text-slate-500 hover:text-slate-700 text-sm">
          Cancel
        </button>
      </div>
    </form>
