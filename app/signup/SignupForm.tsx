...
export function SignupForm({
  ...
  function handleSubmit(formData: FormData) {
    ...
    startTransition(async () => {
      ...
      toast.success(
        `Account created! Redirecting to ${r.tenant_slug}.getrentalflow.com...`,
      );
      if (r.redirect_url) {
        setTimeout(() => {
    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="text-center mb-6">
          ...
          {isBeta ? (
            <div className="mt-3">
              <span className="inline-block bg-brand-yellow text-brand-navy text-[10px] font-bold tracking-widest px-2 py-0.5 rounded mb-1">
                BETA PROGRAM
              </span>
              ...
              <p className="text-xs text-slate-500 mt-1">
                Sign up while the beta is open and the trial doubles to{" "}
                {trialDays} days — no card required.
              </p>
            </div>
          ) : (
