...
// Dollar-quote every text field — no single-quote escape concerns anywhere.
const T = "kbf"; // short tag
...
const inserts = sections.map((sec) => {
  ...
  if (md.includes(`$${T}$`) || sec.title.includes(`$${T}$`)) {
    throw new Error(`Section ${sec.id} contains the dollar tag — pick another.`);
  }
  const slug = `help-${sec.id}`;
  const cat = inferCategory(sec.id, sec.title);
  // One INSERT per section — easier for Postgres to recover from a single
  // bad row, and lets Supabase Editor stream results progressively.
  return `insert into kb_articles (slug, title, body_md, category, tags, is_published) values
  ($${T}$${slug}$${T}$, $${T}$${sec.title}$${T}$, $${T}$${md}$${T}$, $${T}$${cat}$${T}$, array[$${T}$help$${T}$, $${T}$imported$${T}$], true)
