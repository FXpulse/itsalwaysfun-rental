...
**The backup lives in THREE places** (try them in order of survivability):

### 2a. Cloudflare R2 (off-cloud, survivor of Supabase loss)

The weekly-backup cron uploads to R2 alongside the Supabase Storage copy.
This is the source of truth when Supabase itself is the problem.

```bash
# Use AWS CLI with R2 endpoint (S3-compatible)
export AWS_ACCESS_KEY_ID="$R2_ACCESS_KEY_ID"
export AWS_SECRET_ACCESS_KEY="$R2_SECRET_ACCESS_KEY"
ENDPOINT="https://$R2_ACCOUNT_ID.r2.cloudflarestorage.com"

# List backups (newest first)
aws s3 ls "s3://$R2_BUCKET/" --endpoint-url "$ENDPOINT" | sort -r | head -5

# Download the latest
LATEST=$(aws s3 ls "s3://$R2_BUCKET/" --endpoint-url "$ENDPOINT" | sort -r | head -1 | awk '{print $4}')
aws s3 cp "s3://$R2_BUCKET/$LATEST" ./latest-backup.json --endpoint-url "$ENDPOINT"
```

R2 env vars come from Vercel: `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET`.

### 2b. Supabase Storage (fallback, same project)

Same backup uploaded to the `backups` bucket inside the Supabase project:

```bash
# Via Supabase dashboard:
#   Storage → backups bucket → download the newest backup-YYYY-MM-DD.json
# Or via CLI:
supabase storage download backups backup-2026-MM-DD.json --linked
```

If the Supabase project is the thing you're trying to escape, this won't help — that's why R2 exists.

### 2c. Supabase managed PITR (continuous, last 7 days)

For partial / recent-data scenarios (not full-project loss):

- Supabase Dashboard → Project Settings → Database → Backups → "Restore from a point in time"
- Pick a timestamp within the retention window (Pro plan: 7 days; higher plans: longer)
- Supabase handles the restore internally

This is the FASTEST recovery for "we lost the last hour of data" scenarios. Use this BEFORE 2a/2b for those cases.

### Format notes

- The R2 + Supabase Storage backups are **JSON dumps** (not pg_dump SQL).
- Generated by `lib/backup.ts → exportFullBackup()` — table-by-table SELECT * with relationships.
- Restore requires loading via the import endpoint or a script (see step 3).

```bash
# If you need to inspect contents
jq 'keys' latest-backup.json     # list tables in the backup
jq '.bookings | length' latest-backup.json  # row count per table
```

---

## Step 3 — Restore data (~30 min)

**⚠ GAP IDENTIFIED 2026-06-16:** there is NO automated restore code. `lib/backup.ts`
has `exportFullBackup()` that generates the JSON, but no matching
`importFullBackup()` to consume it. Restore is currently a manual operation.

### Option A — Schema first (apply migrations to fresh project)

1. On the target Supabase project, apply all SQL from `supabase/` directory:
   ```bash
   # Concatenated migrations:
   psql "$TARGET_DATABASE_URL" < supabase/ALL_MIGRATIONS.sql
   ```
2. This recreates tables, RLS policies, RPCs, triggers — but no data yet.

### Option B — Data load (manual table-by-table from JSON)
...
For each table, you have to:

```bash
# Extract one table to a CSV-friendly format
jq -r '.bookings[] | [.id, .tenant_id, .customer_email, ...] | @csv' latest-backup.json > bookings.csv

# Load into Postgres
psql "$TARGET_DATABASE_URL" -c "\\copy bookings(id, tenant_id, customer_email, ...) FROM 'bookings.csv' CSV"
```

Or write a one-off Node script that reads `latest-backup.json` and does
`supabase.from(table).upsert(rows)` for each table in dependency order
(tenants first, then bookings, then booking_expenses, etc.).

### Option C — Supabase managed PITR (BEST for recent disasters)

For "I deleted the prod table by accident 30 min ago" scenarios — this is the
fastest path and doesn't need any of the JSON-restore work:

- Supabase Dashboard → Project Settings → Database → Point-in-time recovery
- Pick a timestamp BEFORE the incident
- Supabase rolls back the entire DB
